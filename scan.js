...
const directoryPath = path.join(__dirname, 'projects/ngx-success-box/src/lib');
const results = [];
...
try {
  ...
  const fp = fs.createWriteStream(path.join(__dirname, 'projects/ngx-success-box/src/public-api.ts'), {
    flags: 'w'
  });
  results.forEach((str) => {
